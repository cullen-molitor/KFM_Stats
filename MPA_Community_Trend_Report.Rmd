...
RF_Reserve_Model <- randomForest::randomForest(
  ...
  ReserveStatus ~ ., ntree = 7000, mtry = 8,
...
```{r RF Partial Importance}
RF_Importance <- as.data.frame(importance(RF_Reserve_Model)) %>%
...
for (i in seq_along(Important_Variables)) {
      a <- partialPlot(RF_Reserve_Model, as.data.frame(Random_Kelp_Forest),
                Important_Variables[i], xlab = Important_Variables[i],
                main=paste("Partial Dependence on", Important_Variables[i]))
}
is.vector(Important_Variables)
is.matrix(Important_Variables)
```
...
```{r PDO plot, fig.height=4, fig.width=8.5}
