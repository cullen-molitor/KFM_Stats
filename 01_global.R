...
{
  Original_16_Benthic_Biomass_Data <- readr::read_csv("Tidy_Data_Dont_Touch/Original_16_Benthic_Biomass_Long.csv")
}
...
{ # Original 16 Data -----
  Original_16_Data <- readr::read_csv("Tidy_Data_Dont_Touch/Original_16_Benthic.csv") 
}
...
{ # Figures and Tables   ----
  ### The order here is vital due to the plus one nature. 
  ### If A figure moves in the text, it must also be moved HERE!!!
  # EXECCUTTIVE SUMMARY
  # INTRODUCTION  -----
  fig <- 1
  CHIS_map_fig <- paste("Fig ", fig, ".", sep = "")
  fig <- fig + 1
  ONI_fig <- paste("Fig ", fig, ".", sep = "")
  fig <- fig + 1
  PDO_fig <- paste("Fig ", fig, ".", sep = "")
  # METHODS  -----
  fig <- fig + 1
  CHIS_MPA_map_fig <- paste("Fig ", fig, ".", sep = "")
  table <- 1
  Best_Proto_Table <-  paste("Table ", table, ".", sep = "")
  table <- 1
  GLMM_table <-  paste("Table ", table, ".", sep = "")
  # SHANNON INDEX  -----
  fig <- fig + 1
  ...
  # White URCHIN   -----
  fig <- fig + 1
  White_fig <- paste("Fig ", fig, ".", sep = "")
  table <- table + 1
  White_table <- paste("Table ", table, ".", sep = "")
  # PURPLE URCHIN   -----
  fig <- fig + 1
  ...
  # ROCK WRASSE   -----
  fig <- fig + 1
  Halsem_f_fig <- paste("Fig ", fig, ".", sep = "")
  table <- table + 1
  Halsem_f_table <- paste("Table ", table, ".", sep = "")
  fig <- fig + 1
  Halsem_m_fig <- paste("Fig ", fig, ".", sep = "")
  table <- table + 1
  Halsem_m_table <- paste("Table ", table, ".", sep = "")
  # SUNFLOWER STAR   -----
  fig <- fig + 1
  ...
  # FISH BIOMASS RATIOS  -----
  fig <- fig + 1
  FishRatio_fig1 <- paste("Fig ", fig, ".", sep = "")
  fig <- fig + 1
  FishRatio_fig2 <- paste("Fig ", fig, ".", sep = "")
  fig <- fig + 1
  FishRatio_fig3 <- paste("Fig ", fig, ".", sep = "")
  fig <- fig + 1
  FishRatio_fig4 <- paste("Fig ", fig, ".", sep = "")
  # BENTHIC BIOMASS RATIOS ----
  fig <- fig + 1
  BenRatio_fig1 <- paste("Fig ", fig, ".", sep = "")
  fig <- fig + 1
  BenRatio_fig2 <- paste("Fig ", fig, ".", sep = "")
  fig <- fig + 1
  BenRatio_fig3 <- paste("Fig ", fig, ".", sep = "")
  # GIANT-SPINED STAR   -----
  fig <- fig + 1
  ...
  table <- table + 1
  RF_VI_table <- paste("Table ", table, ".", sep = "")
  table <- table + 1
  ISA_table <- paste("Table ", table, ".", sep = "")
  # APPENDIX C. SITE METADATA  -----
  table <- table + 1
  Site_table <- paste("Table ", table, ".", sep = "")
  # APPENDIX D. SPECIES AND ASSOCIATED PROTOCOLS METADATA  -----
  table <- table + 1
  Species_table <- paste("Table ", table, ".", sep = "")
  # APPENDIX #. PROTOCOL   -----
  table <- table + 1
  Protocol_table <- paste("Table ", table, ".", sep = "")
  # APENDIX F. LENGTH TO WEIGHT EQUATIONS  -----
  # FISH LW  ----
  table <- table + 1
  Fish_LW_Summary_table <- paste("Table ", table, ".", sep = "")
  table <- table + 1
  Fish_LW_full_table <- paste("Table ", table, ".", sep = "")
  table <- table + 1
  Fish_Regressiom_table <- paste("Table ", table, ".", sep = "")
  # BENTHIC LW ----
  table <- table + 1
  Ben_LW_Summary_table <- paste("Table ", table, ".", sep = "")
  table <- table + 1
  Ben_LW_full_table <- paste("Table ", table, ".", sep = "")
  table <- table + 1
  Benthic_Regressiom_table <- paste("Table ", table, ".", sep = "")
  table <- table + 1
  Gorgonian_Regressiom_table <- paste("Table ", table, ".", sep = "")
  # APPENDIX F SPECIAL AREAS OF INTEREST  ----
}

{ # Notes  -----
# Green and pink ab biomass? Surely there is data somewhere

# Think about the potential issues of outputs being FSC by month - IE 
# a survey done in june is going to have a lower FSC than a survey done in September. 
# This is within a season and the coefficients look pretty close, so maybe it's not a big deal, 
# but this sounds like maybe not an apples/apples situation. Discussed this with roommates, thought 
# that averaging out the slopes from may-sept might provide a reasonable "season-level" estimate of 
# biomass, given that the SBC model is calculating NPP based on the notion of mass-specific growth 
# rates, and FSC is broken up into submerged and canopy sections (or combined to whole FSC which 
# are the coefficients used here), so while stipe count doesn't increase by much within a season, 
# the total biomass associated with that individual over the summer's growth greatly increases 
# (obviously submerged portion on canopy forming individuals doesn't change much as depth is 
# fixed, but a larger proportion of the individual is on the surface). I think we can
# basically "convert it to midsummer" using the average coefficient over the season, allowing 
# for between site comparisons within a year. 6/24/20 I did it this way for now as a 
# placeholder 6/30 we think this is defensible

# 6/30/2020 is this worth splitting hairs over density vs biomass for inverts? 
# I initially say yes because that indicates some size-structure differences 
# (ie recruitment events if you had a density spike but not much biomass change),
# but maybe we don't care about this as much for inverts, 

# 6/30/2020 calculate biomass ratios inside/outside MPAs like Josh did for fish 
# in his 2016 talk. One could argue that those plots could replace everything
# we've done for biomass but that doesn't really show the effects of reserve 
# status and island over time like our current GLMMs and plot sets do.

# Similarly, I want to combine the benthic and fish diversity by site and year
# with kelp biomass, but I don't calculate those until later.    

# 6/24/20 I added this below fish diversity (since that's the first point running from
# top down where I have the data I want, even though it feels like it should be up here in the "cleanup")

# 7/9 Diversity and Community Similarity have consumed us and this has become the main thrust
# of the first part of the results, as of 7/9 we're joining all the diversity and similarity 
# stuff between benthic and fish.
}
